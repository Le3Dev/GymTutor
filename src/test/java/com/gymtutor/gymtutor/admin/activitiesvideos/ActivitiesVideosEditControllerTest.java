...
class ActivitiesVideosEditControllerTest {
    ...
    void ct01() throws Exception {
        doNothing().when(videosService).updateVideo(any(), any(Integer.class));

        mockMvc.perform(post("/admin/activities/{activityId}/videos/{videoId}/edit", ACTIVITY_ID, VIDEO_ID)
                        .param("videoName", "abc")
                        .param("videoLink", "https://www.youtube.com/watch?v=abc123")
                        .with(csrf()))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/admin/activities/1/videos"))
                .andExpect(flash().attribute("successMessage", "Video alterado com sucesso!!!"));
    }
    ...
    void ct09() throws Exception {
        doCallRealMethod().when(videosService).updateVideo(any(), any(Integer.class));
